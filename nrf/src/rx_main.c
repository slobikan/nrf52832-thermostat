...
static uint32_t g_uart_rx_last_byte_ms;
    ...
    for (token = work; *token != '\0'; ) {
        ...
        if (*sep == '=') {
            ...
            *sep++ = '\0';
            ...
            } else if ((strcmp(token, "T") == 0) || (strcmp(token, "TARGET") == 0) || (strcmp(token, "SETPOINT") == 0)) {
                g_cmd_setpoint = parse_temp_value(sep);
                g_cmd_setpoint_valid = (g_cmd_setpoint != 0u);
                apply_output_state();
                uart_publish_status();
            } else if ((strcmp(token, "M") == 0) || (strcmp(token, "MODE") == 0)) {
                if ((sep[0] == 'c') || (sep[0] == 'C')) {
...
static void uart_poll_commands(void)
{
    ...
    if ((g_uart_rx_len > 0u) && ((millis() - g_uart_rx_last_byte_ms) >= RX_UART_FRAME_TIMEOUT_MS)) {
        g_uart_rx_buf[g_uart_rx_len] = '\0';
        handle_uart_command(g_uart_rx_buf);
        g_uart_rx_len = 0u;
    }
