...
enum {
    GLASS_SEG_LEFT_ADEF  = 0u,
    GLASS_SEG_LEFT_BGC   = 1u,
    GLASS_SEG_RIGHT_ADEF = 2u,
    GLASS_SEG_RIGHT_BGC  = 3u
};
...
static void display_set_pixel(uint8_t glass_seg, uint8_t glass_com, bool on)
{
    tm1621b_set(glass_seg, glass_com, on);
}

static void display_show_mask_pair(uint8_t left_mask, uint8_t right_mask, bool show_degree)
{
    if ((left_mask & SEG_A) != 0u) {
        display_set_pixel(GLASS_SEG_LEFT_ADEF, 0u, true);
    }
    if ((right_mask & SEG_A) != 0u) {
        display_set_pixel(GLASS_SEG_RIGHT_ADEF, 0u, true);
    }
    if (show_degree) {
        display_set_pixel(GLASS_SEG_LEFT_BGC, 0u, true);
    }

    if ((left_mask & SEG_F) != 0u) {
        display_set_pixel(GLASS_SEG_LEFT_ADEF, 1u, true);
    }
    if ((left_mask & SEG_B) != 0u) {
        display_set_pixel(GLASS_SEG_LEFT_BGC, 1u, true);
    }
    if ((right_mask & SEG_F) != 0u) {
        display_set_pixel(GLASS_SEG_RIGHT_ADEF, 1u, true);
    }
    if ((right_mask & SEG_B) != 0u) {
        display_set_pixel(GLASS_SEG_RIGHT_BGC, 1u, true);
    }

    if ((left_mask & SEG_E) != 0u) {
        display_set_pixel(GLASS_SEG_LEFT_ADEF, 2u, true);
    }
    if ((left_mask & SEG_G) != 0u) {
        display_set_pixel(GLASS_SEG_LEFT_BGC, 2u, true);
    }
    if ((right_mask & SEG_E) != 0u) {
        display_set_pixel(GLASS_SEG_RIGHT_ADEF, 2u, true);
    }
    if ((right_mask & SEG_G) != 0u) {
        display_set_pixel(GLASS_SEG_RIGHT_BGC, 2u, true);
    }

    if ((left_mask & SEG_D) != 0u) {
        display_set_pixel(GLASS_SEG_LEFT_ADEF, 3u, true);
    }
    if ((left_mask & SEG_C) != 0u) {
        display_set_pixel(GLASS_SEG_LEFT_BGC, 3u, true);
    }
    if ((right_mask & SEG_D) != 0u) {
        display_set_pixel(GLASS_SEG_RIGHT_ADEF, 3u, true);
    }
    if ((right_mask & SEG_C) != 0u) {
        display_set_pixel(GLASS_SEG_RIGHT_BGC, 3u, true);
    }
}

static void display_show_digits(uint8_t left_digit, uint8_t right_digit, bool show_degree)
{
    ...
    display_show_mask_pair(left_mask, right_mask, show_degree);
}

static void display_show_pair_p(bool show_on)
{
    if (!show_on) {
        return;
    }
...
static void display_show_co(bool show_on)
{
    if (!show_on) {
        return;
    }
...
static void display_refresh(void)
{
    ...
    if (g_mode == TX_MODE_PAIRING) {
        display_show_pair_p(g_blink_on);
    } else if (g_mode == TX_MODE_CO) {
        display_show_co(g_blink_on);
    } else if (g_mode == TX_MODE_CONNECTED_HOLD) {
        display_show_digits(9u, 9u, false);
    } else if (g_mode == TX_MODE_SETPOINT) {
        if (g_blink_on) {
            display_show_digits((uint8_t)(g_setpoint / 10u),
                                (uint8_t)(g_setpoint % 10u),
                                false);
        }
    } else {
        display_temp = (uint8_t)(((uint32_t)g_last_temp_x10 + 5u) / 10u);
        display_show_digits((uint8_t)(display_temp / 10u),
...
static void update_temp_if_due(void)
{
    ...
    new_temp = sample_temp_x10();
    if (new_temp != g_last_temp_x10) {
        g_last_temp_x10 = new_temp;
        if (g_mode == TX_MODE_SHOW_TEMP) {
            display_mark_dirty();
        }
    }
}
...
static bool try_pair_exchange(void)
{
    ...
    while ((millis() - wait_start) < TX_ACK_WAIT_MS) {
        if (radio_rx_packet_lcd_friendly(&ack, TX_ACK_SLICE_MS, &crc_ok) && crc_ok
            ...
            g_last_telem_ms = g_success_start_ms;
            g_blink_on = true;
            display_mark_dirty();
            return true;
        }
